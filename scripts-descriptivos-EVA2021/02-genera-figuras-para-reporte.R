  ...
  data %>% 
    mutate(dif = {{posl}} - lag({{posl}}, order_by = {{xxn}}), 
           poslabel_aj = case_when(
             dif < 3 & {{freq}} >= 3 ~ {{posl}} + 3, 
             dif < 3 & {{freq}} < 3 ~ {{posl}} + 2, 
             dif < 3 & {{freq}} < 2 ~ {{posl}} + 1.3, 
             dif < 3 & {{freq}} < 1.5 ~ {{posl}} + 1, 
             TRUE ~ {{posl}}), 
...
#generar figuras para el reporte descriptivo -----

#load(here("3-reportes", "1-descriptivos", "01-descriptivos-ffaa.Rdata"))
load(here("3-reportes", "1-descriptivos", "01-descriptivos-usab.Rdata"))
...
#redondeamos
tabla3 <- mutate(tabla3, prop = round(prop, 1))

# (2) generamos graficos -----

nom <- unique(tabla3$Concatena1)

# (2.1) graficos de barras, un solo item ----

for(i in 1:length(nom)){ #i=8
  
  bd <- filter(tabla3, Concatena1 == nom[i], TipoV == "Categorico1", estrato == "General")
  ...
  for(j in 1:length(vars1)){ #j=1
    ...
    g <-
      ...
      geom_bar(stat = "identity", position = "dodge", fill = "#8dd3c7")  + 
    ...
    ggsave(
      here("3-reportes", "1-descriptivos", "2-con-pesos", "0-figuras", paste0("g_", nom[i], "_", vars1[j], ".pdf")),
      g1,
...
# (2.2) graficos de barras, varios item ----

for(i in 1:length(nom)){ #i=4
  
  bd <- filter(tabla3, Concatena1 == nom[i], TipoV == "Categorico2", estrato == "General")
